...
def cached_resample(hr_bytes, hr_name):
    ...
    dfs, _ = parse_timestamps({"heartrate": df})
    hr_df = dfs["heartrate"]
    result, _ = resample_heartrate(hr_df)
    return result
...
# ── Load + merge (cached — only runs once per upload set) ──────
status_ph = st.empty()

def show_loading(msg):
    status_ph.markdown(f'''
    <div class="loading-banner">
        <div style="font-size:1.1rem;margin-bottom:8px">⏳ <strong style="color:#a78bfa">{msg}</strong></div>
        <div class="prog-wrap"><div class="prog-fill"></div></div>
        <div style="font-size:0.8rem;color:#5a7a9a;margin-top:6px">Please wait — do not refresh the page</div>
    </div>''', unsafe_allow_html=True)

show_loading("Step 1 of 3 — Reading files and parsing timestamps…")
dfs = cached_load(file_keys)

show_loading("Step 2 of 3 — Merging all 5 files into one dataset…")
master_df = cached_master(file_keys)

hr_resampled = None
if "heartrate" in file_keys:
    show_loading("Step 3 of 3 — Resampling heart rate to 1-minute intervals…")
...
feat_matrix = build_clustering_features(master_df)
...
k_range_list, inertias = cached_elbow(feat_pkl, k_max=10)
...
with st.expander("📈  Step 4 — Trend Forecasting (Prophet)"):
    ...
    def prophet_plot(ds_series, y_series, label, key, unit="", color="#10b981"):
        ...
        col_btn, col_sl = st.columns([2, 1])
        days = col_sl.slider("Days to forecast", 7, 90, 30, key=f"sl_{key}")
        run  = col_btn.button(f"▶️  Forecast {label}", key=f"btn_{key}")
        ...
        if saved:
            ...
            # 1. CI band — shaded ribbon over full forecast range
            fig.add_trace(go.Scatter(
                ...
                name="80% CI",
                hovertemplate="Upper: %{y:.1f}<extra>80% CI</extra>",
            ))
            ...
            fig.add_trace(go.Scatter(
                ...
                name="Trend",
                hovertemplate="Date: %{x|%Y-%m-%d}<br>Trend: %{y:.1f}<extra>Trend</extra>",
            ))
            ...
            fig.add_trace(go.Scatter(
                ...
                name=y_lbl,
                hovertemplate="Date: %{x|%Y-%m-%d}<br>" + label + ": %{y:.1f}<extra>" + y_lbl + "</extra>",
            ))
    ...
    if hr_resampled is not None and "Time" in hr_resampled.columns:
        ...
        hr_p = hr_resampled.copy()
        hr_p["Time"] = pd.to_datetime(hr_p["Time"], errors="coerce")
        # Aggregate to DAILY average — Prophet needs daily granularity
        hr_p["date"] = hr_p["Time"].dt.date
        hr_agg = hr_p.groupby("date")["Value"].mean().reset_index()
        hr_agg["date"] = pd.to_datetime(hr_agg["date"])
        prophet_plot(hr_agg["date"], hr_agg["Value"], "Heart Rate", "hr", "bpm", color="#38bdf8")
    else:
        st.info("Heart rate file not uploaded.")
...
with st.expander("🔵  Step 5 — User Grouping (Clustering)"):
    ...
    n_k   = cc1.slider("KMeans — number of groups (k)", 2, min(10, len(feat_scaled)-1), 3, key="k",
                        help="How many groups should KMeans create?")
    eps   = cc2.slider("DBSCAN — neighbourhood size", 0.3, 5.0, 1.5, 0.1, key="eps",
                        help="How close two users need to be to join the same group")
    msamp = cc3.slider("DBSCAN — min group size", 2, 15, 3, key="ms",
                        help="A group needs at least this many users to count")

    # KMeans & DBSCAN — fast, no cache needed (milliseconds)
    ...
    master_df["KMeans_Cluster"] = km_labels
    master_df["DBSCAN_Cluster"] = db_labels
    ...
    with p2:
        ...
        fig_db = px.scatter(pca_df, x="PC1", y="PC2", color="DBSCAN",
                            ...
                            color_discrete_sequence=["#ef4444","#a78bfa","#38bdf8","#10b981","#f59e0b","#6366f1"])
        fig_db.update_traces(marker=dict(size=13, opacity=0.85, line=dict(color="white", width=1)))
        st.plotly_chart(T(fig_db), use_container_width=True)

    tip("**Outlier** (shown in red in DBSCAN) = a user whose habits don't fit any group — could be a data issue or a genuinely unusual person.")
    ...
    if "tsne_df" not in st.session_state:
        ...
        with t2:
            fig_t2 = px.scatter(tsne_df, x="tSNE1", y="tSNE2", color="DBSCAN",
                                ...
                                color_discrete_sequence=["#ef4444","#a78bfa","#38bdf8","#10b981","#f59e0b","#6366f1"])
            fig_t2.update_traces(marker=dict(size=12, opacity=0.85, line=dict(color="white", width=1)))
    ...
    num_cols = [c for c in master_df.select_dtypes(include=[np.number]).columns
    ...
    if plot_cols:
        profile  = master_df.groupby("KMeans_Cluster")[plot_cols].mean().reset_index()
    ...
    for i, row in master_df.groupby("KMeans_Cluster")[plot_cols if plot_cols else num_cols].mean().iterrows():
        parts = []
        ...
        n_users = int((master_df["KMeans_Cluster"] == i).sum())
